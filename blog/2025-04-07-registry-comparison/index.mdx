...
Based on the original Docker runtime and image format, the Open Container Initiative (OCI) stepped in to standardize these into open, vendor-neutral specifications: the OCI Image, Distribution, and Runtime specs. This brought consistency and portability to container images, and now, the same is happening for other types of software artifacts.

Popular tools like Helm, Docker, and [ORAS](https://oras.land/) support pushing and pulling OCI-compliant artifacts. A growing trend is using ORAS to package and store non-OCI-native formats, like Kubeflow pipelines, configuration bundles, or machine learning models, as OCI artifacts.
...
```bash
oras push registry.distr.sh/my-org/ml-model:latest ./model.pkl:application/vnd.myorg.model
```

Think of an OCI registry as a standardized grocery bag, it expects items to be packed in a particular way. ORAS is the delivery service that takes whatever file you give it, regardless of format, and packages it properly so it fits in the bag. No need to worry about the wrapping yourself; ORAS handles that, making sure your artifact can be stored, versioned, and retrieved just like a container image.
## Key Considerations when choosing a registry

When evaluating registries, consider these core criteria:

**Internal vs. External Use**  
Is this for your internal development team's workflow, or for distributing to customers? This is the most fundamental distinction.
...
For internal use, integration with your existing IAM may be sufficient. For external distribution, you'll need granular controls over which artifacts are visible to which customers. Vulnerability scanning is also an important consideration.
